...
func (svc *GatewayService) OnIncomingConnection(conn net.Conn) (session gateway.IGatewaySession) {
	//var err error
	newSession, err := StartGatewaySession(svc.resolverPath, svc.authnSvc)
...
func NewGatewayService(resolverPath string, authnSvc authn.IAuthnService) *GatewayService {
	svc := &GatewayService{
