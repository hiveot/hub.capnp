...
plugins: logger smbserver ## Build the plugins for amd64 and arm

logger smbserver:
	GOOS=linux GOARCH=amd64 $(GOBUILD) -o $(X64_DIST_FOLDER)/$@ plugins/$@/cmd/main.go
	GOOS=linux GOARCH=arm $(GOBUILD) -o $(ARM_DIST_FOLDER)/$@ plugins/$@/cmd/main.go
