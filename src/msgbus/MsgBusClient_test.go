...
func TestConnect(t *testing.T) {
	...
	err = msgbus.Publish(conn, channel1, []byte("Hello world"))
	assert.NoError(t, err)
...
func TestNewPubSubErrors(t *testing.T) {
	...
	serverCertPath := path.Join(certFolder, msgbus.ServerCertFile)
	clientCertPath := path.Join(certFolder, msgbus.ClientCertFile)
	clientKeyPath := path.Join(certFolder, msgbus.ClientKeyFile)
	...
	_, err := msgbus.NewWebsocketConnection(hostPort, client1ID, nil)
	require.Error(t, err)

	_, err = msgbus.NewTLSWebsocketConnection(hostPort, client1ID, nil, clientCertPEM, clientKeyPEM, serverCertPEM)
	require.Error(t, err)
...
func TestBadPublish(t *testing.T) {
	...
	c, _ := msgbus.NewWebsocketConnection(hostPort, client1ID, nil)
	require.Nil(t, c)
