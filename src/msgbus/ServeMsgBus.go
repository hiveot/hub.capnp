// Package msgbus with simple internal message bus for serving plugins pub/sub
package msgbus
...
// ServeMsgBus provides a lightweight server for managing multiple pub/sub channels
type ServeMsgBus struct {
...
func (mbs *ServeMsgBus) AddConnection(c *websocket.Conn) {
...
//  channelID to add
func (mbs *ServeMsgBus) AddChannel(channelID string) *Channel {
	mbs.updateMutex.Lock()
	defer mbs.updateMutex.Unlock()
...
func (mbs *ServeMsgBus) AddSubscriber(channelID string, conn *websocket.Conn) *Channel {
...
func (mbs *ServeMsgBus) GetChannel(channelID string) *Channel {
	mbs.updateMutex.Lock()
	defer mbs.updateMutex.Unlock()
	channel := mbs.subscriptions[channelID]
	return channel
...
func (mbs *ServeMsgBus) GetSubscribers(channelID string) []*websocket.Conn {
	mbs.updateMutex.Lock()
	defer mbs.updateMutex.Unlock()
	channel := mbs.subscriptions[channelID]
	if channel == nil {
...
func (mbs *ServeMsgBus) PublishToSubscribers(channelID string, message []byte) {
...
func (mbs *ServeMsgBus) RemoveConnection(c *websocket.Conn) {
...
func (mbs *ServeMsgBus) RemoveSubscriber(channelID string, c *websocket.Conn) {
...
func (mbs *ServeMsgBus) sendChannelMessageToSubscribers(channelID string, message []byte) {
	...
	for _, c := range consumers {
		err := send(c, MsgBusCommandReceive, channelID, message)
		if err != nil {
...
func (mbs *ServeMsgBus) serveConnection(response http.ResponseWriter, request *http.Request) {
	...
	go func() {
		listen(c, func(command string, topic string, data []byte) {
			if data == nil {
				mbs.RemoveConnection(c)
			} else if command == MsgBusCommandPublish {
				mbs.PublishToSubscribers(topic, data)
			} else if command == MsgBusCommandSubscribe {
				mbs.AddSubscriber(topic, c)
			} else if command == MsgBusCommandUnsubscribe {
...
func (mbs *ServeMsgBus) Start(host string) (*mux.Router, error) {
	var err error
...
func NewServeMsgBus() *ServeMsgBus {
	p := &ServeMsgBus{
