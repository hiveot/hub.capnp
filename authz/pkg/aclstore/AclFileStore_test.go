...
func TestSetRoleAndRestart(t *testing.T) {
	...
	// cleanup
	err = aclStore.SetRole(user1, group2, authorize.GroupRoleNone)
	err = aclStore.SetRole(user2, group2, authorize.GroupRoleNone)
...
func TestCompareRoles(t *testing.T) {
	...
	ge = aclstore.IsRoleGreaterEqual(authorize.GroupRoleOperator, authorize.GroupRoleViewer)
	assert.True(t, ge)
	ge = aclstore.IsRoleGreaterEqual(authorize.GroupRoleViewer, authorize.GroupRoleOperator)
	assert.False(t, ge)

	ge = aclstore.IsRoleGreaterEqual(authorize.GroupRoleManager, authorize.GroupRoleOperator)
	assert.True(t, ge)
	ge = aclstore.IsRoleGreaterEqual(authorize.GroupRoleOperator, authorize.GroupRoleManager)
	assert.False(t, ge)
