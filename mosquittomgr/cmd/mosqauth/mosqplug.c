...
#endif
{
  ...
  const char *subjname = "";
  X509 *cert = mosquitto_client_certificate(client); // if client uses cert auth
  if (cert != NULL)
  {
    subjname = X509_NAME_oneline(X509_get_subject_name(cert), NULL, 0);
  }

  if (username == NULL || (password == NULL && subjname == NULL))
  {
  ...
  GoUint8 ret = AuthUnpwdCheck(go_clientid, go_username, go_password, go_clientip, go_subjname);
  printf("mosquitto_auth_unpwd_check: username=%s, subjname=%s, result=%d\n", username, subjname, ret);
  X509_free(cert);
  return ret;
}
...
#endif
{
  ...
  if (cert != NULL)
  {
    ...
    certSubjname = X509_NAME_oneline(subject, NULL, 0);
  }
  ...
  GoString go_certSubjname = {certSubjname, strlen(certSubjname)};
  // GoString go_certCN = {certCN, strlen(certCN)};

  // ssl testing
  printf("mosquitto_auth_acl_check: clientId=%s, certCN=%s\n", clientid, certSubjname);
  fflush(stdout);
  //---
  GoUint8 ret = AuthAclCheck(go_clientid, go_username, go_certSubjname, go_topic, go_access);
  X509_free(cert);
  return ret;
}
