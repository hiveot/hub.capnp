...
func TestConnectNoTLS(t *testing.T) {
	...
	cs, err := smbserver.Start(hostPort)
...
func TestConnectNoWSClient(t *testing.T) {
	...
	srv, err := smbserver.Start(hostPort)
	require.NoError(t, err)
...
func TestStartTwice(t *testing.T) {
	...
	cs1, err := smbserver.Start(hostPort)
	require.NoError(t, err)
	cs2, err := smbserver.Start(hostPort)
	time.Sleep(time.Second)
...
func TestPubSub(t *testing.T) {
	...
	mb, err := smbserver.Start(hostPort)
	require.NoError(t, err)
	time.Sleep(time.Second)
...
func TestTLSPubSub(t *testing.T) {
	...
	cs, err := smbserver.StartTLS(hostPort, certFolder)
	require.NoError(t, err)
