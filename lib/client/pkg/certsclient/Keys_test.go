...
// TestMain clears the certsclient folder for clean testing of keys and certsclient
...
func TestSaveLoadPrivKey(t *testing.T) {
	...
	err := certsclient.SaveKeysToPEM(privKey, privKeyPemFile)
	assert.NoError(t, err)

	privKey2, err := certsclient.LoadKeysFromPEM(privKeyPemFile)
	assert.NoError(t, err)
...
func TestSaveLoadPrivKeyNotFound(t *testing.T) {
	...
	//
	privKey2, err := certsclient.LoadKeysFromPEM("/root")
	assert.Error(t, err)
