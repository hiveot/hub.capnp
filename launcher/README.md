...
For systemd installation to run as user 'wost'. When changing the user and folders make sure to edit the init/wost-hub.service file accordingly. From the dist folder run:
...
The Hub looks in the {home}/config folder for hub.yaml. The file MUST exist as it contains the message bus connection information for use by plugins. By default the host outbound IP address is used. For hosts with multiple addresses, the
...
The Hub can be launched manually by invoking the 'hub' app in the wost bin folder. eg ~/bin/wost/bin/hub. The services to start are defined in the hub.yaml configuration file.
...
This turns the 3rd party devices into WoST Things. For example, an openzwave protocol binding turns ZWave devices into WoST compatible Things.
